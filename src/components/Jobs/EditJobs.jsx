...
import { AddNewJob, useCompanyDetails, useJobDetails  } from "../../Hooks/Company";
...
const EditJobs = () => {
    ...
    const { data , isLoading} = useCompanyDetails(company?.company._id)
    ...
    jobDetails?.qualification.map((qualification) => {
      qualificationValues.push(qualification)
    })
    
    const initialState = { jobTitle: jobDetails?.jobTitle, jobCategory : jobDetails?.jobCategory , minExp  : jobDetails?.minExp ,maxExp  : jobDetails?.maxExp ,timeSchedule  : jobDetails?.timeSchedule ,minSalary  : jobDetails?.minSalary ,maxSalary  : jobDetails?.maxSalary ,qualification  : jobDetails?.qualification ,education  : jobDetails?.education ,jobLocation  : jobDetails?.jobLocation ,skills  : jobDetails?.skills ,language  : jobDetails?.language , status : false}
    
    const [formData, setFormData] = useState(initialState)
    console.log("h1" , formData?.qualification);
    const [qualificationValues, setqualificationValues] = useState(formData.qualification)

    console.log("h3" , formData?.qualification);
    console.log("h4" , jobDetails?.qualification);
useEffect(() => {
    setErrors('')
    ...
    return (
        ...
        <div className="container mx-auto mt-10 max-w-screen-lg mx-auto">
          <h4 className="text-3xl font-semibold">
            Edit <span className="text-primary">{jobDetails?.jobTitle}</span>
          </h4>
                  ...
                  <div className="w-4/5">
                    <div class="flex flex-wrap -mx-3 mb-6">
                      ...
                      {qualificationValues.map((element,index) => (
                       ...
                       value={element.qualification || formData.qualification }
                     />
